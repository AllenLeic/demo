...
public final class MailUtil {
	...
	public static void sendMail(String toEmail, String emailMsg) throws Exception {
		...
		props.put("mail.smtp.host", "localhost");// 这里改为服务器地址
		props.put("mail.smtp.auth", "true");
		Authenticator auth = new Authenticator() {
			public PasswordAuthentication getPasswordAuthentication() {
				return new PasswordAuthentication("leicong", "ddd");// 这里写发件人账号和密码,不要@ 如果有授权码,就不用密码
			}
		};
		Session session = Session.getInstance(props, auth);
		//2_创建一封邮件
		Message message = new MimeMessage(session);
		message.setFrom(new InternetAddress("leicong@itheima.com")); // 这里写发件人邮箱账户完整名称
	...
	public static void main(String[] args) throws Exception{
		String toEmail = "leicong@itheima.com"; 
